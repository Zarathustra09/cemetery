                                    ...
                                    <?php for ($j = 1; $j <= $table_column; $j++) { 
                                        ...
                                        while ($row1 = $result1->fetch_assoc()) {
                                            if($row1['status'] == 'NOT AVAILABLE'){
                                                $button = '<td>
                                                <button  class="btn btn-danger button-slot" name="'.$slot.'"><i class="fas fa-times"></i></button>
                                            </td>';
                                            }
                                            else if($row1['status'] == 'RESERVED' && $row1['user_id'] == $_SESSION['user_id'] ){
                                                $button = '<td>
                                            ...
                                            else if($row1['status'] == 'TAKEN'  && $row1['user_id'] == $_SESSION['user_id']  ){
                                                $button = '<td>
                                                ...
                                                $button = '<td>
                                                <button class="btn btn-secondary button-slot" name="'.$slot.'&type=' . $_GET["type"] . '"><i class="fas fa-lock"></i></button>
                                            </td>';
                                            }
                                            else if($row1['status'] == 'TAKEN'  && $row1['user_id'] != $_SESSION['user_id']  ){
                                                $button = '<td>
                                                <button class="btn btn-secondary button-slot" name="'.$slot.'&type=' . $_GET["type"] . '"><i class="fas fa-user"></i></button>
                                            </td>';
                                            }
                                        }
                                    ?>
...
graves.forEach(function(grave) {
    ...
    let btn_reserve = `<a href="register.php?slot_value=${grave.slot_value}&grave_id=${grave.grave_id}&transaction=edit&type=${grave.type}"><span class="badge badge-info"><h5>RESERVE<h5></span></a>`;
...
legend.innerHTML = `
    ...
    <div>
        <span style="background-color: blue; width: 15px; height: 15px; display: inline-block; margin-right: 5px;"></span>
        AVAILABLE
    </div>
    <div>
        <span style="background-color: orange; width: 15px; height: 15px; display: inline-block; margin-right: 5px;"></span>
        RESERVED
    </div>
    <div>
    ...
    <div>
        <span style="background-color: gray; width: 15px; height: 15px; display: inline-block; margin-right: 5px;"></span>
        TAKEN BY OTHERS
    </div>
`;
